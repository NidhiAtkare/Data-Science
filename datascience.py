...
TASK 1: Data Loading & Cleaning and Data Preprocessing:
...
"""Observations from Distribution Plots — Crime Dataset:
 ...
 * Skewed Distribution:
Crime data is not uniform — values are clustered near the lower range, with a long tail toward higher crime numbers.
...
plt.title("Crime Share Distribution — India 2019")
plt.show()
...
plt.title("Investigation Status — India 2019")
...
print("\n✅ Analysis Completed — Charts Generated Successfully\n")
...
# Violent Crime Index (example crime col names—adjust based on your dataset)
...
TASK 5: Classification Modelling:
"""
...
"""Converted crime counts into two categories — High Crime and Low Crime — using median thresholding.
 ...
 * Helps identify the optimal number of clusters by finding the curve “bend”
...
sns.scatterplot(
    data=df,
    x='Chargesheets submitted - Cases Chargesheeted',
    y='Cases Pending Investigation at End of Year',
    hue='cluster',
    s=120
)
plt.title('K-Means Clustering — Chargesheeted vs Pending at End')
plt.xlabel('Cases Chargesheeted')
...
"""📊 Interpretation of K-Means Clustering on Crime Investigation Metrics (k = 3)
...
🔹 Three Distinct Crime Pattern Clusters
...
* Cluster 0 — High Crime Load / High Pending Cases
...
* Cluster 1 — Low Crime Volume / Efficient Resolution
...
* Cluster 2 — Moderate Crime, Higher Chargesheet Efficiency
...
Interpretation:
These may be mid-crime regions showing good investigation efficiency—able to process many cases despite workload.

TASK 7: DBSCAN (Advanced k-means)
"""
...
plt.subplot(2, 2, 1)
sns.scatterplot(
    x='Cases Pending Investigation (Prev Year)',
...
sns.scatterplot(
    x='Cases Pending Investigation (Prev Year)',
    y='Chargesheets submitted - Cases Chargesheeted',
    hue='cluster',
    data=crime_clustering_features,
    palette='viridis',
    s=50, alpha=0.6
...
shortened_crime_features = {
    'Cases Pending Investigation (Prev Year)': 'Pending (Prev Year)',
    'Cases Reported during the year': 'Reported (Current Year)',
    'Chargesheets submitted - Cases Chargesheeted': 'Chargesheeted',
    'Cases Pending Investigation at End of Year': 'Pending (End Year)'
}
...
plt.figure(figsize=(15, 10))
plt.suptitle('DBSCAN Clustering — Crime Dataset', fontsize=16)
...
"""✅ Interpretation of Clusters based on Mean Crime Feature Values
...
Interpretation:
Districts with high crime rates and slow investigation/clearance — possible police resource strain.
...
Interpretation:
Major crime-dense districts or urban hubs — high activity + backlog despite high case processing.
 ...
 * Pending (End Year)
are extremely high relative to the dataset’s overall mean and upper quartile (75th percentile).
...
✅ DBSCAN Clustering Results — Crime Dataset
...
Cluster 0 — Majority / Normal Crime Pattern
...
Cluster 1 — High Pending Case / Backlog Cluster
...
Cluster 2 — High Efficiency / Case Disposal Cluster
...
Cluster 3 — Crime Hotspot / High Activity Cluster
 ...
 * K-Means assigned every record to a cluster, whereas DBSCAN marked some records as noise (-1) — meaning unusual or extreme crime cases.
...
st.write("Dataset Loaded and Processed Successfully ✅")
...
fig_bar = px.bar(
    df,
    y=selected_crime,
    title=f"{selected_crime} — Bar Chart"
)
...
st.subheader("📎 Crime Feature Comparison")
...
# === Save all Matplotlib plots into one PDF and download ===
...
pdf_path = f"outputs/all_plots_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"

# Create a multi-page PDF for all current figures
with PdfPages(pdf_path) as pdf:
    figs = [plt.figure(i) for i in plt.get_fignums()]
    if len(figs) == 0:
        print("⚠️ No active plots found. Run your plotting cells first.")
    else:
        for fig in figs:
            pdf.savefig(fig)
        print(f"✅ Saved {len(figs)} plots to {pdf_path}")

# --- For Google Colab: download the file automatically ---
try:
    from google.colab import files
    files.download(pdf_path)
except Exception as e:
    print(f"File saved locally at {pdf_path}. Download manually if not in Colab.")
